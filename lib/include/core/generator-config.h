...
#define MSDLIB_PUBLIC// for DLL import/export
#endif
...
struct ErrorCorrectionConfig
{
  ...
  static MSDLIB_PUBLIC const double defaultMinDeviationRatio;
  /// The default value of minImproveRatio.
  static MSDLIB_PUBLIC const double defaultMinImproveRatio;
  ...
  /// Configuration of whether to use an algorithm that computes the exact shape distance at the positions of suspected
  /// artifacts. This algorithm can be much slower.
  ...
  /// The minimum ratio between the pre-correction distance error and the post-correction distance error. Has no effect
  /// for DO_NOT_CHECK_DISTANCE.
  double minImproveRatio;
  /// An optional buffer to avoid dynamic allocation. Must have at least as many bytes as the MSDF has pixels.
  byte *buffer;

  inline explicit ErrorCorrectionConfig(Mode mode = EDGE_PRIORITY,
    DistanceCheckMode distanceCheckMode = CHECK_DISTANCE_AT_EDGE,
    double minDeviationRatio = defaultMinDeviationRatio,
    double minImproveRatio = defaultMinImproveRatio,
    byte *buffer = nullptr)
    : mode(mode), distanceCheckMode(distanceCheckMode), minDeviationRatio(minDeviationRatio),
      minImproveRatio(minImproveRatio), buffer(buffer)
  {}
};
...
struct GeneratorConfig
{
  ...
  /// May be set to false to improve performance when no such contours are present.
  bool overlapSupport;
...
struct MSDFGeneratorConfig : GeneratorConfig
{
  ...
  inline explicit MSDFGeneratorConfig(bool overlapSupport,
    const ErrorCorrectionConfig &errorCorrection = ErrorCorrectionConfig())
