...
namespace msdfgen {
/// Performs error correction on a computed MSDF to eliminate interpolation artifacts. This is a low-level class, you
/// may want to use the API in msdf-error-correction.h instead.
...
public:
  ...
  template<int N> void protectEdges(const BitmapConstRef<float, N> &sdf);
  ...
  template<int N> void findErrors(const BitmapConstRef<float, N> &sdf);
  /// Flags texels that are expected to cause interpolation artifacts based on analysis of the SDF and comparison with
  /// the exact shape distance.
  ...
  template<int N> void apply(const BitmapRef<float, N> &sdf) const;
