...
public:
  ...
  /// Sets whether the origin point should be at the same position in each glyph, separately for horizontal and vertical
  /// dimension
  ...
  /// Outputs the position of the origin within each cell, each value is only valid if the origin is fixed in the
...
private:
  ...
  msdfgen::Shape::Bounds getMaxBounds(double &maxWidth,
    double &maxHeight,
    GlyphGeometry *glyphs,
    int count,
    double scale,
    double range) const;
  double scaleToFit(GlyphGeometry *glyphs,
    int count,
    int cellWidth,
    int cellHeight,
