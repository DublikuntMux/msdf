...
static int ftMoveTo(const FT_Vector *to, void *user)
{
  ...
  if (!(context->contour && context->contour->edges.empty())) context->contour = &context->shape->addContour();
...
FT_Error readFreetypeOutline(Shape &output, FT_Outline *outline)
{
  ...
  if (!output.contours.empty() && output.contours.back().edges.empty()) output.contours.pop_back();
  return error;
}
...
bool loadGlyph(Shape &output, FontHandle *font, GlyphIndex glyphIndex, double *advance)
{
  ...
  if (error) return false;
  if (advance) *advance = F26DOT6_TO_DOUBLE(font->face->glyph->advance.x);
...
{
  return getKerning(output,
    font,
    GlyphIndex(FT_Get_Char_Index(font->face, unicode0)),
    GlyphIndex(FT_Get_Char_Index(font->face, unicode1)));
}
...
bool setFontVariationAxis(FreetypeHandle *library, FontHandle *font, const char *name, double coordinate)
{
  ...
  if (font->face->face_flags & FT_FACE_FLAG_MULTIPLE_MASTERS) {
    ...
    if (FT_Get_MM_Var(font->face, &master)) return false;
    if (master && master->num_axis) {
      ...
      if (FT_Set_Var_Design_Coordinates(font->face, FT_UInt(coords.size()), &coords[0])) success = false;
    }
