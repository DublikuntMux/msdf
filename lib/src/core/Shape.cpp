...
void Shape::addContour(Contour &&contour) { contours.push_back((Contour &&)contour); }
...
static void deconvergeEdge(EdgeHolder &edgeHolder, int param)
{
  ...
  case (int)CubicSegment::EDGE_TYPE:
    static_cast<CubicSegment *>(&*edgeHolder)->deconverge(param, MSDLIB_DECONVERGENCE_FACTOR);
  }
}
  ...
  for (std::vector<Contour>::iterator contour = contours.begin(); contour != contours.end(); ++contour) {
    if (contour->edges.size() == 1) {
      ...
      for (std::vector<EdgeHolder>::iterator edge = contour->edges.begin(); edge != contour->edges.end(); ++edge) {
        ...
        if (dotProduct(prevDir, curDir) < MSDLIB_CORNER_DOT_EPSILON - 1) {
...
void Shape::boundMiters(double &l, double &b, double &r, double &t, double border, double miterLimit, int polarity)
  const
{
  for (std::vector<Contour>::const_iterator contour = contours.begin(); contour != contours.end(); ++contour)
...
Shape::Bounds Shape::getBounds(double border, double miterLimit, int polarity) const
{
  ...
  if (border > 0) {
    ...
    if (miterLimit > 0) boundMiters(bounds.l, bounds.b, bounds.r, bounds.t, border, miterLimit, polarity);
  }
  return bounds;
}
...
void Shape::orientContours()
{
  ...
  const double ratio =
    .5 * (sqrt(5) - 1);// an irrational number to minimize chance of intersecting a corner or other point of interest
  ...
  for (int i = 0; i < (int)contours.size(); ++i) {
    if (!orientations[i] && !contours[i].edges.empty()) {
      ...
      double y1 = y0;
      for (std::vector<EdgeHolder>::const_iterator edge = contours[i].edges.begin();
           edge != contours[i].edges.end() && y0 == y1;
           ++edge)
        y1 = (*edge)->point(1).y;
      for (std::vector<EdgeHolder>::const_iterator edge = contours[i].edges.begin();
           edge != contours[i].edges.end() && y0 == y1;
      ...
      for (int j = 0; j < (int)contours.size(); ++j) {
        for (std::vector<EdgeHolder>::const_iterator edge = contours[j].edges.begin(); edge != contours[j].edges.end();
             ++edge) {
          int n = (*edge)->scanlineIntersections(x, dy, y);
          for (int k = 0; k < n; ++k) {
  ...
  for (int i = 0; i < (int)contours.size(); ++i)
    if (orientations[i] < 0) contours[i].reverse();
}
