...
{
  return sign(
    reinterpret_cast<const Scanline::Intersection *>(a)->x - reinterpret_cast<const Scanline::Intersection *>(b)->x);
}
...
void Scanline::preprocess()
{
  ...
  if (!intersections.empty()) {
    ...
    for (std::vector<Intersection>::iterator intersection = intersections.begin(); intersection != intersections.end();
         ++intersection) {
...
int Scanline::moveTo(double x) const
{
  ...
  if (x < intersections[index].x) {
    ...
    while (index < (int)intersections.size() - 1 && x >= intersections[index + 1].x) ++index;
  }
  lastIndex = index;
  return index;
}

int Scanline::countIntersections(double x) const { return moveTo(x) + 1; }

int Scanline::sumIntersections(double x) const
{
  int index = moveTo(x);
  if (index >= 0) return intersections[index].direction;
  return 0;
}

bool Scanline::filled(double x, FillRule fillRule) const { return interpretFillRule(sumIntersections(x), fillRule); }
