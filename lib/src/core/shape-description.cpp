...
#include "core/shape-description.hpp"
...
static int readControlPoints(T *input, Point2 *output)
{
  ...
  if (result == 2) {
    ...
    if (result == 2 && readChar(input) == ')') return 2;
...
static bool readContour(T *input, Contour &output, const Point2 *first, int terminator, bool &colorsSpecified)
{
  ...
  if (first)
    ...
    if (result != 2) return result != 1 && readChar(input) == terminator;
  }
  start = p[0];
  int c = '\0';
  while ((c = readChar(input)) != terminator) {
    ...
    if (result == 2) {
      ...
      case '(':
      READ_CONTROL_POINTS:
        if ((controlPoints = readControlPoints<T, readChar, readCoord>(input, p + 1)) < 0) return false;
        break;
      default:
        return false;
      }
      if (readChar(input) != ';') return false;
    FINISH_EDGE:
...
bool readShapeDescription(FILE *input, Shape &output, bool *colorsSpecified)
{
  ...
  if (result == 2) {
    ...
    for (; c == '{'; c = readCharF(input))
      if (!readContour<FILE, readCharF, readCoordF>(input, output.addContour(), NULL, '}', locColorsSpec)) return false;
    if (colorsSpecified) *colorsSpecified = locColorsSpec;
  ...
  if (result == 2) {
    ...
    if (c == '@') {
      for (int i = 0; i < (int)sizeof("invert-y") - 1; ++i)
        if (input[i] != "invert-y"[i]) return false;
      output.inverseYAxis = true;
...
static bool isColored(const Shape &shape)
{
  for (std::vector<Contour>::const_iterator contour = shape.contours.begin(); contour != shape.contours.end();
    ...
    for (std::vector<EdgeHolder>::const_iterator edge = contour->edges.begin(); edge != contour->edges.end(); ++edge)
      if ((*edge)->color != WHITE) return true;
  return false;
}
...
bool writeShapeDescription(FILE *output, const Shape &shape)
{
  ...
  if (shape.inverseYAxis) fprintf(output, "@invert-y\n");
  for (std::vector<Contour>::const_iterator contour = shape.contours.begin(); contour != shape.contours.end();
       ++contour) {
    fprintf(output, "{\n");
    if (!contour->edges.empty()) {
      for (std::vector<EdgeHolder>::const_iterator edge = contour->edges.begin(); edge != contour->edges.end();
        ...
        case (int)QuadraticSegment::EDGE_TYPE:
          ...
          writeCoord(output, p[0]);
          fprintf(output, ";\n\t\t");
          if (colorCode) fprintf(output, "%c", colorCode);
          fprintf(output, "(");
