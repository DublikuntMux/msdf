...
public:
  ...
  /// Evaluates if the median value xm interpolated at xt in the range between am at at and bm at bt indicates an
  /// artifact.
  inline int rangeTest(double at, double bt, double xt, float am, float bm, float xm) const
  {
    ...
    if ((am > .5f && bm > .5f && xm <= .5f) || (am < .5f && bm < .5f && xm >= .5f)
        || (!protectedFlag && median(am, bm, xm) != xm)) {
      double axSpan = (xt - at) * span, bxSpan = (bt - xt) * span;
      // Check if the interpolated median's value is in the expected range based on its distance (span) from boundaries
      // a, b.
  ...
  /// Returns true if the combined results of the tests performed on the median value m interpolated at t indicate an
  /// artifact.
  inline bool evaluate(double t, float m, int flags) const { return (flags & 2) != 0; }
...
/// The shape distance checker evaluates the exact shape distance to find additional artifacts at a significant
/// performance cost.
...
public:
  ...
  public:
    ...
    inline bool evaluate(double t, float m, int flags) const
    {
      if (flags & CLASSIFIER_FLAG_CANDIDATE) {
        // Skip expensive distance evaluation if the point has already been classified as an artifact by the base
        // classifier.
        if (flags & CLASSIFIER_FLAG_ARTIFACT) return true;
        ...
        // Compute the color that would be interpolated at the artifact candidate's position if error correction was
        // applied on the current texel.
        ...
        // Compute the evaluated distance (interpolated median) before and after error correction, as well as the exact
        // shape distance.
        float oldPSD = median(oldMSD[0], oldMSD[1], oldMSD[2]);
        float newPSD = median(newMSD[0], newMSD[1], newMSD[2]);
        float refPSD = float(
          parent->invRange * parent->distanceFinder.distance(parent->shapeCoord + tVector * parent->texelSize) + .5);
  ...
  const float *msd;
  bool protectedFlag;
  inline ShapeDistanceChecker(const BitmapConstRef<float, N> &sdf,
    const Shape &shape,
    const Projection &projection,
    double invRange,
    double minImproveRatio)
    : distanceFinder(shape), sdf(sdf), invRange(invRange), minImproveRatio(minImproveRatio)
  {
...
MSDFErrorCorrection::MSDFErrorCorrection(const BitmapRef<byte, 1> &stencil, const Projection &projection, double range)
  : stencil(stencil), projection(projection)
{
  invRange = 1 / range;
...
void MSDFErrorCorrection::setMinImproveRatio(double minImproveRatio) { this->minImproveRatio = minImproveRatio; }
    ...
    if (!contour->edges.empty()) {
      ...
      for (std::vector<EdgeHolder>::const_iterator edge = contour->edges.begin(); edge != contour->edges.end();
        ...
        if (!(commonColor & (commonColor - 1))) {
          ...
          if (shape.inverseYAxis) p.y = stencil.height - p.y;
          ...
          if (l < stencil.width && b < stencil.height && r >= 0 && t >= 0) {
            if (l >= 0 && b >= 0) *stencil(l, b) |= (byte)PROTECTED;
            if (r < stencil.width && b >= 0) *stencil(r, b) |= (byte)PROTECTED;
            if (l >= 0 && t < stencil.height) *stencil(l, t) |= (byte)PROTECTED;
            if (r < stencil.width && t < stencil.height) *stencil(r, t) |= (byte)PROTECTED;
          }
        }
        prevEdge = *edge;
      }
...
{
  return (RED * edgeBetweenTexelsChannel(a, b, 0) + GREEN * edgeBetweenTexelsChannel(a, b, 1)
...
template<int N> void MSDFErrorCorrection::protectEdges(const BitmapConstRef<float, N> &sdf)
{
  float radius;
...
void MSDFErrorCorrection::protectAll()
{
  ...
  for (byte *mask = stencil.pixels; mask < end; ++mask) *mask |= (byte)PROTECTED;
}
...
{
  return (
    // For protected texels, only report an artifact if it would cause fill inversion (change between positive and
    // negative distance).
    (!isProtected || (am > .5f && bm > .5f && xm <= .5f) || (am < .5f && bm < .5f && xm >= .5f)) &&
    // This is an artifact if the interpolated median is outside the range of possible values based on its distance from
    // a, b.
    !(xm >= am - axSpan && xm <= am + axSpan && xm >= bm - bxSpan && xm <= bm + bxSpan));
}

/// Checks if a linear interpolation artifact will occur at a point where two specific color channels are equal - such
/// points have extreme median values.
template<class ArtifactClassifier>
static bool hasLinearArtifactInner(const ArtifactClassifier &artifactClassifier,
  float am,
  float bm,
  const float *a,
  const float *b,
...
/// Checks if a bilinear interpolation artifact will occur at a point where two specific color channels are equal - such
/// points have extreme median values.
template<class ArtifactClassifier>
static bool hasDiagonalArtifactInner(const ArtifactClassifier &artifactClassifier,
  float am,
  float dm,
  const float *a,
  const float *l,
  ...
  double tEx1)
{
  ...
  for (int i = 0; i < solutions; ++i) {
    // Solutions t[i] == 0 and t[i] == 1 are singularities and occur very often because two channels are usually equal
    // at texels.
    if (t[i] > ARTIFACT_T_EPSILON && t[i] < 1 - ARTIFACT_T_EPSILON) {
      ...
      if (artifactClassifier.evaluate(t[i], xm, rangeFlags)) return true;
    }
  }
  return false;
}
...
/// Checks if a bilinear interpolation artifact will occur inbetween two diagonally adjacent texels a, d (with b, c
/// forming the other diagonal).
template<class ArtifactClassifier>
static bool hasDiagonalArtifact(const ArtifactClassifier &artifactClassifier,
  float am,
  const float *a,
  const float *b,
  const float *c,
  const float *d)
{
  ...
  if (fabsf(am - .5f) >= fabsf(dm - .5f)) {
    ...
    // Compute interpolation ratios tEx (0 < tEx[i] < 1) for the local extremes of each color channel (the derivative
    // 2*q[i]*tEx[i]+l[i] == 0).
    double tEx[3] = { -.5 * l[0] / q[0], -.5 * l[1] / q[1], -.5 * l[2] / q[2] };
    // Check points where each pair of color channels meets.
    return (
      hasDiagonalArtifactInner(
        artifactClassifier, am, dm, a, l, q, a[1] - a[0], b[1] - b[0] + c[1] - c[0], d[1] - d[0], tEx[0], tEx[1])
      || hasDiagonalArtifactInner(
        artifactClassifier, am, dm, a, l, q, a[2] - a[1], b[2] - b[1] + c[2] - c[1], d[2] - d[1], tEx[1], tEx[2])
      || hasDiagonalArtifactInner(
        artifactClassifier, am, dm, a, l, q, a[0] - a[2], b[0] - b[2] + c[0] - c[2], d[0] - d[2], tEx[2], tEx[0]));
  }
  return false;
}

template<int N> void MSDFErrorCorrection::findErrors(const BitmapConstRef<float, N> &sdf)
{
  ...
  for (int y = 0; y < sdf.height; ++y) {
    for (int x = 0; x < sdf.width; ++x) {
      ...
      // Mark current texel c with the error flag if an artifact occurs when it's interpolated with any of its 8
      // neighbors.
      *stencil(x, y) |=
        (byte)(ERROR
               * ((x > 0
                    && ((l = sdf(x - 1, y)), hasLinearArtifact(BaseArtifactClassifier(hSpan, protectedFlag), cm, c, l)))
                  || (y > 0
                      && ((b = sdf(x, y - 1)),
                        hasLinearArtifact(BaseArtifactClassifier(vSpan, protectedFlag), cm, c, b)))
                  || (x < sdf.width - 1
                      && ((r = sdf(x + 1, y)),
                        hasLinearArtifact(BaseArtifactClassifier(hSpan, protectedFlag), cm, c, r)))
                  || (y < sdf.height - 1
                      && ((t = sdf(x, y + 1)),
                        hasLinearArtifact(BaseArtifactClassifier(vSpan, protectedFlag), cm, c, t)))
                  || (x > 0 && y > 0
                      && hasDiagonalArtifact(
                        BaseArtifactClassifier(dSpan, protectedFlag), cm, c, l, b, sdf(x - 1, y - 1)))
                  || (x < sdf.width - 1 && y > 0
                      && hasDiagonalArtifact(
                        BaseArtifactClassifier(dSpan, protectedFlag), cm, c, r, b, sdf(x + 1, y - 1)))
                  || (x > 0 && y < sdf.height - 1
                      && hasDiagonalArtifact(
                        BaseArtifactClassifier(dSpan, protectedFlag), cm, c, l, t, sdf(x - 1, y + 1)))
                  || (x < sdf.width - 1 && y < sdf.height - 1
                      && hasDiagonalArtifact(
                        BaseArtifactClassifier(dSpan, protectedFlag), cm, c, r, t, sdf(x + 1, y + 1)))));
    }
  }
}
  ...
  double dSpan = minDeviationRatio * projection.unprojectVector(Vector2(invRange)).length();
  {
    ...
    for (int y = 0; y < sdf.height; ++y) {
      ...
      for (int col = 0; col < sdf.width; ++col) {
        ...
        if ((*stencil(x, row) & ERROR)) continue;
        ...
        *stencil(x, row) |=
          (byte)(ERROR
                 * ((x > 0
                      && ((l = sdf(x - 1, row)),
                        hasLinearArtifact(shapeDistanceChecker.classifier(Vector2(-1, 0), hSpan), cm, c, l)))
                    || (row > 0
                        && ((b = sdf(x, row - 1)),
                          hasLinearArtifact(shapeDistanceChecker.classifier(Vector2(0, -1), vSpan), cm, c, b)))
                    || (x < sdf.width - 1
                        && ((r = sdf(x + 1, row)),
                          hasLinearArtifact(shapeDistanceChecker.classifier(Vector2(+1, 0), hSpan), cm, c, r)))
                    || (row < sdf.height - 1
                        && ((t = sdf(x, row + 1)),
                          hasLinearArtifact(shapeDistanceChecker.classifier(Vector2(0, +1), vSpan), cm, c, t)))
                    || (x > 0 && row > 0
                        && hasDiagonalArtifact(
                          shapeDistanceChecker.classifier(Vector2(-1, -1), dSpan), cm, c, l, b, sdf(x - 1, row - 1)))
                    || (x < sdf.width - 1 && row > 0
                        && hasDiagonalArtifact(
                          shapeDistanceChecker.classifier(Vector2(+1, -1), dSpan), cm, c, r, b, sdf(x + 1, row - 1)))
                    || (x > 0 && row < sdf.height - 1
                        && hasDiagonalArtifact(
                          shapeDistanceChecker.classifier(Vector2(-1, +1), dSpan), cm, c, l, t, sdf(x - 1, row + 1)))
                    || (x < sdf.width - 1 && row < sdf.height - 1
                        && hasDiagonalArtifact(
                          shapeDistanceChecker.classifier(Vector2(+1, +1), dSpan), cm, c, r, t, sdf(x + 1, row + 1)))));
      }
    }
  }
}

template<int N> void MSDFErrorCorrection::apply(const BitmapRef<float, N> &sdf) const
{
...
BitmapConstRef<byte, 1> MSDFErrorCorrection::getStencil() const { return stencil; }
...
template void MSDFErrorCorrection::findErrors<SimpleContourCombiner>(const BitmapConstRef<float, 3> &sdf,
  const Shape &shape);
template void MSDFErrorCorrection::findErrors<SimpleContourCombiner>(const BitmapConstRef<float, 4> &sdf,
  const Shape &shape);
template void MSDFErrorCorrection::findErrors<OverlappingContourCombiner>(const BitmapConstRef<float, 3> &sdf,
  const Shape &shape);
template void MSDFErrorCorrection::findErrors<OverlappingContourCombiner>(const BitmapConstRef<float, 4> &sdf,
  const Shape &shape);
