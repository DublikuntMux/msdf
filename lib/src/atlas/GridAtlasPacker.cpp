...
{
  if (squareConstraint(constraint)) width = height = std::min(width, height);
  switch (constraint) {
  case DimensionsConstraint::NONE:
  case DimensionsConstraint::SQUARE:
    break;
  ...
  case DimensionsConstraint::POWER_OF_TWO_RECTANGLE:
  case DimensionsConstraint::POWER_OF_TWO_SQUARE:
    if (width > 0) width = floorPOT(width);
    if (height > 0) height = floorPOT(height);
    break;
  }
}
...
{
  if (squareConstraint(constraint)) width = height = std::max(width, height);
  switch (constraint) {
  case DimensionsConstraint::NONE:
  case DimensionsConstraint::SQUARE:
    break;
  ...
  case DimensionsConstraint::POWER_OF_TWO_RECTANGLE:
  case DimensionsConstraint::POWER_OF_TWO_SQUARE:
    if (width > 0) width = ceilPOT(width);
    if (height > 0) height = ceilPOT(height);
    break;
  }
}
...
GridAtlasPacker::GridAtlasPacker()
  : columns(-1), rows(-1), width(-1), height(-1), cellWidth(-1), cellHeight(-1), spacing(0),
    dimensionsConstraint(DimensionsConstraint::NONE), cellDimensionsConstraint(DimensionsConstraint::NONE),
    hFixed(false), vFixed(false), scale(-1), minScale(1), fixedX(0), fixedY(0), unitRange(0), pxRange(0), miterLimit(0),
    pxAlignOriginX(false), pxAlignOriginY(false), scaleMaximizationTolerance(.001), alignedColumnsBias(.125),
    cutoff(false)
{}

msdfgen::Shape::Bounds GridAtlasPacker::getMaxBounds(double &maxWidth,
  double &maxHeight,
  GlyphGeometry *glyphs,
  int count,
  double scale,
  double range) const
{
  ...
  for (GlyphGeometry *glyph = glyphs, *end = glyphs + count; glyph < end; ++glyph) {
    if (!glyph->isWhitespace()) {
      ...
      if (miterLimit > 0) glyph->getShape().boundMiters(l, b, r, t, .5 * shapeRange, miterLimit, 1);
  ...
  if (maxBounds.l >= maxBounds.r || maxBounds.b >= maxBounds.t) maxBounds = msdfgen::Shape::Bounds();
  // If origin is pixel-aligned but not fixed, one pixel has to be added to max dimensions to allow for aligning the
  // origin by shifting by < 1 pixel
  if (hFixed)
...
double GridAtlasPacker::scaleToFit(GlyphGeometry *glyphs,
  int count,
  int cellWidth,
  int cellHeight,
  ...
  double &maxHeight) const
{
  ...
  if (cellWidth <= 0) cellWidth = BIG_VALUE;
  if (cellHeight <= 0) cellHeight = BIG_VALUE;
  --cellWidth, --cellHeight;// Implicit half-pixel padding from each side to make sure that no representable values are
                            // beyond outermost pixel centers
...
#define TRY_FIT(scale)                                                                                    \
  ...
  if (TRY_FIT(1)) {
    while (maxScale < 1e+32 && ((maxScale = 2 * minScale), TRY_FIT(maxScale))) minScale = maxScale;
  } else {
    while (minScale > 1e-32 && ((minScale = .5 * maxScale), !TRY_FIT(minScale))) maxScale = minScale;
  }
  ...
  if (!lastResult) TRY_FIT(minScale);
  return minScale;
}

// Can this spaghetti code be simplified?
// Idea: Maybe it could be rewritten into a while (not all properties deduced) cycle, and compute one value in each
// iteration
int GridAtlasPacker::pack(GlyphGeometry *glyphs, int count)
{
  ...
  if (columns > 0 && rows > 0)
    ...
    for (const GlyphGeometry *glyph = glyphs, *end = glyphs + count; glyph < end; ++glyph) {
      if (!glyph->isWhitespace()) ++cellCount;
    }
    if (columns > 0)
  ...
  if (height < 0 && cellHeight > 0 && rows > 0) height = rows * cellHeight;
  if (width != initial.width || height != initial.height) raiseToConstraint(width, height, dimensionsConstraint);

  if (cellWidth < 0 && width > 0 && columns > 0) cellWidth = (width + spacing) / columns;
  if (cellHeight < 0 && height > 0 && rows > 0) cellHeight = (height + spacing) / rows;
  if (cellWidth != initial.cellWidth || cellHeight != initial.cellHeight) {
    ...
    if ((cellWidth == 0 && positiveCellWidth) || (cellHeight == 0 && positiveCellHeight)) return -1;
  }

  if ((cellWidth > 0 && cellWidth - spacing - 1 <= pxRange)
      || (cellHeight > 0 && cellHeight - spacing - 1 <= pxRange))// cells definitely too small
    return -1;
  ...
  if (scale <= 0) {
    ...
    if (pxRange && miterLimit > 0) {
      ...
      if (cellWidth > 0 || cellHeight > 0) {
        ...
        if (!bestCols) return -1;
        // If columns can be aligned with total width at a slight cost to glyph scale, use that number of columns
        // instead
      ...
      if (scale <= 0) {
        ...
        if (scale < minScale)
          maxBounds =
            getMaxBounds(maxWidth, maxHeight, glyphs, count, scale = minScale, unitRange + pxRange / minScale);
      }
      ...
      if (cellWidth > 0) hScale = (cellWidth - hSlack - spacing - 1 - pxRange) / maxWidth;
      if (cellHeight > 0) vScale = (cellHeight - vSlack - spacing - 1 - pxRange) / maxHeight;
      if (hScale || vScale) {
        if (hScale && vScale)
        ...
        double bestAlignedScale = 0;
        int bestCols = 0, bestAlignedCols = 0;
        // TODO optimize to only test up to sqrt(cellCount) cols and rows like in the above branch (for (int q = (int)
        // sqrt(cellCount)+1; ...)
      ...
      if (initial.rows < 0 && initial.cellHeight < 0) {
        int optimalCellWidth = cellWidth,
            optimalCellHeight = (int)ceil(scale * maxHeight + pxRange) + vSlack + spacing + 1;
        raiseToConstraint(optimalCellWidth, optimalCellHeight, cellDimensionsConstraint);
        if (optimalCellHeight < cellHeight && optimalCellWidth <= cellWidth) {
          cellWidth = optimalCellWidth;
          cellHeight = optimalCellHeight;
  ...
  if (rows * cellHeight > height) rows = height / cellHeight;

  int col = 0, row = 0;
  for (GlyphGeometry *glyph = glyphs, *end = glyphs + count; glyph < end; ++glyph) {
    if (!glyph->isWhitespace()) {
      glyph->frameBox(scale,
        unitRange + pxRange / scale,
        miterLimit,
        cellWidth - spacing,
        cellHeight - spacing,
        hFixed ? &fixedX : nullptr,
        vFixed ? &fixedY : nullptr,
        pxAlignOriginX,
        pxAlignOriginY);
      glyph->placeBox(col * cellWidth, height - (row + 1) * cellHeight);
      if (++col >= columns) {
        if (++row >= rows) { return end - glyph - 1; }
        col = 0;
      }
    }
  }
...
void GridAtlasPacker::setFixedOrigin(bool horizontal, bool vertical) { hFixed = horizontal, vFixed = vertical; }

void GridAtlasPacker::setCellDimensions(int width, int height) { cellWidth = width, cellHeight = height; }

void GridAtlasPacker::unsetCellDimensions() { cellWidth = -1, cellHeight = -1; }
...
void GridAtlasPacker::setColumns(int columns) { this->columns = columns; }

void GridAtlasPacker::setRows(int rows) { this->rows = rows; }

void GridAtlasPacker::unsetColumns() { columns = -1; }

void GridAtlasPacker::unsetRows() { rows = -1; }

void GridAtlasPacker::setDimensions(int width, int height) { this->width = width, this->height = height; }

void GridAtlasPacker::unsetDimensions() { width = -1, height = -1; }
...
void GridAtlasPacker::setSpacing(int spacing) { this->spacing = spacing; }

void GridAtlasPacker::setScale(double scale) { this->scale = scale; }

void GridAtlasPacker::setMinimumScale(double minScale) { this->minScale = minScale; }

void GridAtlasPacker::setUnitRange(double unitRange) { this->unitRange = unitRange; }

void GridAtlasPacker::setPixelRange(double pxRange) { this->pxRange = pxRange; }

void GridAtlasPacker::setMiterLimit(double miterLimit) { this->miterLimit = miterLimit; }

void GridAtlasPacker::setOriginPixelAlignment(bool align) { pxAlignOriginX = align, pxAlignOriginY = align; }
...
void GridAtlasPacker::getDimensions(int &width, int &height) const { width = this->width, height = this->height; }

void GridAtlasPacker::getCellDimensions(int &width, int &height) const { width = cellWidth, height = cellHeight; }

int GridAtlasPacker::getColumns() const { return columns; }

int GridAtlasPacker::getRows() const { return rows; }

double GridAtlasPacker::getScale() const { return scale; }

double GridAtlasPacker::getPixelRange() const { return pxRange + scale * unitRange; }
...
bool GridAtlasPacker::hasCutoff() const { return cutoff; }
}// namespace msdf_atlas
