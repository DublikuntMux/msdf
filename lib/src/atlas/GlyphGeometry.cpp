...
bool GlyphGeometry::load(msdfgen::FontHandle *font,
  ...
  if (font && msdfgen::loadGlyph(shape, font, index, &advance) && shape.validate()) {
    ...
    bounds = shape.getBounds();
    {
      ...
      if (msdfgen::SimpleTrueShapeDistanceFinder::oneShotDistance(shape, outerPoint) > 0) {
        for (msdfgen::Contour &contour : shape.contours) contour.reverse();
      }
    }
    return true;
  }
...
void GlyphGeometry::edgeColoring(void (*fn)(msdfgen::Shape &, double, unsigned long long),
...
void GlyphGeometry::wrapBox(double scale, double range, double miterLimit, bool pxAlignOriginX, bool pxAlignOriginY)
{
  ...
  if (bounds.l < bounds.r && bounds.b < bounds.t) {
    ...
    l -= .5 * range, b -= .5 * range;
    r += .5 * range, t += .5 * range;
    if (miterLimit > 0) shape.boundMiters(l, b, r, t, .5 * range, miterLimit, 1);
    if (pxAlignOriginX) {
...
void GlyphGeometry::frameBox(double scale,
  double range,
  double miterLimit,
  int width,
  int height,
...
msdfgen::GlyphIndex GlyphGeometry::getGlyphIndex() const { return msdfgen::GlyphIndex(index); }
...
const msdfgen::Shape::Bounds &GlyphGeometry::getShapeBounds() const { return bounds; }
...
void GlyphGeometry::getBoxSize(int &w, int &h) const { w = box.rect.w, h = box.rect.h; }
...
msdfgen::Vector2 GlyphGeometry::getBoxTranslate() const { return box.translate; }
...
GlyphGeometry::operator GlyphBox() const
{
  ...
  box.rect.x = this->box.rect.x, box.rect.y = this->box.rect.y, box.rect.w = this->box.rect.w,
  box.rect.h = this->box.rect.h;
  return box;
}
