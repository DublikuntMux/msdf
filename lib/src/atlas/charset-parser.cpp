...
bool Charset::load(const char *filename, bool disableCharLiterals)
{
  ...
  enum { CLEAR, TIGHT, RANGE_BRACKET, RANGE_START, RANGE_SEPARATOR, RANGE_END } state = CLEAR;
  ...
  for (int c = fgetc(f), start = true; c >= 0; start = false) {
    ...
    case '8':
    case '9':// number
      if (!(state == CLEAR || state == RANGE_BRACKET || state == RANGE_SEPARATOR)) goto FAIL;
      buffer.push_back((char)c);
      c = readWord(buffer, f);
      {
        int cp;
        if (!parseInt(cp, buffer.c_str())) goto FAIL;
        switch (state) {
        case CLEAR:
          if (cp >= 0) add((unicode_t)cp);
          state = TIGHT;
          break;
        case RANGE_BRACKET:
        ...
        case RANGE_SEPARATOR:
          for (unicode_t u = rangeStart; (int)u <= cp; ++u) add(u);
          state = RANGE_END;
          break;
        default:;
        }
    ...
    case '\'':// single UTF-8 character
      if (!(state == CLEAR || state == RANGE_BRACKET || state == RANGE_SEPARATOR) || disableCharLiterals) goto FAIL;
      if (!readString(buffer, f, '\'')) goto FAIL;
      utf8Decode(unicodeBuffer, buffer.c_str());
      if (unicodeBuffer.size() == 1) {
        switch (state) {
        case CLEAR:
          if (unicodeBuffer[0] > 0) add(unicodeBuffer[0]);
          state = TIGHT;
          break;
        case RANGE_BRACKET:
          ...
          break;
        case RANGE_SEPARATOR:
          for (unicode_t u = rangeStart; u <= unicodeBuffer[0]; ++u) add(u);
          state = RANGE_END;
          break;
        default:;
        }
    ...
    case '"':// string of UTF-8 characters
      if (state != CLEAR || disableCharLiterals) goto FAIL;
      if (!readString(buffer, f, '"')) goto FAIL;
      utf8Decode(unicodeBuffer, buffer.c_str());
      for (unicode_t cp : unicodeBuffer) add(cp);
      unicodeBuffer.clear();
      buffer.clear();
      state = TIGHT;
      break;
    case '[':// character range start
      if (state != CLEAR) goto FAIL;
      state = RANGE_BRACKET;
      break;
    ...
    case '\r':
    case '\t':// whitespace
      if (state == TIGHT) state = CLEAR;
      break;
    case 0xef:// UTF-8 byte order mark
      if (start) {
        if (!(fgetc(f) == 0xbb && fgetc(f) == 0xbf)) goto FAIL;
        break;
      }
