...
{
  if (minArea > 0) lowerBound = int(sqrt(minArea - 1)) / MULTIPLE + 1;
  updateCurrent();
}
...
template<int MULTIPLE> bool SquareSizeSelector<MULTIPLE>::operator()(int &width, int &height) const
{
  width = MULTIPLE * current, height = MULTIPLE * current;
  return lowerBound < upperBound || upperBound < 0;
}

template<int MULTIPLE> SquareSizeSelector<MULTIPLE> &SquareSizeSelector<MULTIPLE>::operator++()
{
...
{
  while (side * side < minArea) side <<= 1;
}
